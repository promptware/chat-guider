import z from "zod";
import { tool } from 'ai';
import { compileFixup, type Domain, type FieldRule, type ValidationResult, type ValidationSpec } from './validation.js';
...
type MkToolParams<D extends DomainMap, LooseSchema extends z.ZodTypeAny, OutputSchema extends z.ZodTypeAny> = {
  schema: z.ZodType<D>;
...
type AreAllFieldsAdded<All extends PropertyKey, Added extends PropertyKey> = Exclude<All, Added> extends never ? true : false;
...
> = {
  field: <K extends Exclude<keyof D, Added>,
  ...
  const t: ToolLike<LooseSchema, typeof wrappedOutputSchema> = {
    ...
    execute: async (input: z.infer<LooseSchema>, options: unknown) => {
      ...
      if (result.tag === 'rejected') {
        return { tag: 'rejected' as const, validationResults: (result as any).validationResults } as z.infer<typeof wrappedOutputSchema>;
      }
...
export function mkTool<
  ...
  LooseSchema extends z.ZodTypeAny,
  OutputSchema extends z.ZodTypeAny
>(params: MkToolParams<D, LooseSchema, OutputSchema>): BuilderApi<D, LooseSchema, OutputSchema, never> {
  ...
  const api: BuilderApi<D, LooseSchema, OutputSchema, never> = {
    field: (key, cfg) => {
      const normalizedCfg: FieldRule<D, typeof key> = {
        requires: [...(cfg.requires as readonly (Exclude<keyof D, typeof key>)[])] as unknown as Exclude<keyof D, typeof key>[],
        influencedBy: [...(cfg.influencedBy as readonly (Exclude<keyof D, typeof key>)[])] as unknown as Exclude<keyof D, typeof key>[],
